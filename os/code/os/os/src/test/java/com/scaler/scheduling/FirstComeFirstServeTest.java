...
public class FirstComeFirstServeTest {
    ...
    public void test() {
        ...
        // List<Integer> actualCompletionTimes =
        // scheduledProcesses.stream().map(ScheduledProcess::getProcess)
        // .map(IncomingProcess::getCompletedAt))
        // .collect(Collectors.toList());

        // assertEquals("The completion time of the processes should be the same as the
